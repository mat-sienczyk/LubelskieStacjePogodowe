...
class Converter {
    ...
    val dataArrayType = object : TypeToken<List<DataModelModel>>() {}.type
    ...
    @TypeConverter
    fun chartArrayToString(data: List<DataModelModel>?): String? {
        return when (data) {
            null -> null
    ...
    @TypeConverter
    fun stringToChartArray(json: String?): List<DataModelModel>? {
        return when (json) {
            null -> null
